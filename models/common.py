...
class TimeoutExecutor:
    def __init__(self, func, timeout):
        ...
        self.exception = None
        if self.timeout > 0:
            raise TimeoutError("Function closed")
    ...
    def execute(self):
        ...
        if thread.is_alive():
            ...
            raise TimeoutError("Function execution")
...
class CSPLayer(nn.Module):
    def __init__(self, c, num_heads):
        ...
        self.cbam = self._create_cbam(c)
        self.sa = self._create_sa(c)
        self.deform_conv = self._create_deform_conv(c)
        self.gate_attention = self._create_gate_attention(c)
        self.feature_enhancer = self._create_feature_enhancer(c)
        self.residual_path = self._create_residual_path(c)
...
class CSPBlock(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers):
        ...
        self.tr = nn.Sequential(*(CSPLayer(c2, num_heads) for _ in range(num_layers)))
        self.c2 = c2
